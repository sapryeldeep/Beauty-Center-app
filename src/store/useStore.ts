...
export const useStore = create<AppState>((setStore, get) => {
  // Listen to Firebase
  const dataRef = ref(db, 'shamelBeautyCenterPlatformV26');
  ...
  return {
    ...
    updateData: (newData) => {
      ...
      // Async save
      set(ref(db, 'shamelBeautyCenterPlatformV26'), updated).catch(console.error);
    },
    
    saveDataToFirebase: () => {
      set(ref(db, 'shamelBeautyCenterPlatformV26'), get().data).catch(console.error);
    },
    ...
    resetData: () => {
      ...
      set(ref(db, 'shamelBeautyCenterPlatformV26'), cleanData).catch(console.error);
    }
  };
});
