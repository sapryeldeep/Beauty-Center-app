...
export default function AppointmentsTab() {
  ...
  const handleSendWhatsappReminder = (appt: Appointment) => {
    ...
    alert(`📱 [محاكاة الإرسال عبر الواتس آب بنجاح]
مرسل من الرقم المعتمد للمركز: ${senderNumber}
إلى جوال العميل: ${appt.phone}

نص رسالة التذكير المخصصة للمركز:
"${messageText}"`);
  };
    ...
    <div className="space-y-6 animate-fadeIn">
      ...
      <div className="bg-white rounded-2xl p-5 shadow-sm border border-slate-200 flex flex-wrap justify-between items-center gap-4">
        ...
        <div className="flex items-center gap-2 bg-indigo-50 border border-indigo-100 px-4 py-2 rounded-xl text-xs">
          ...
          <span className="font-bold text-indigo-900">رابط الحجز الأونلاين:</span>
          <button 
            onClick={() => {
              navigator.clipboard.writeText(publicBookingUrl);
              alert('تم نسخ رابط الحجز الأونلاين بنجاح! يمكنك مشاركته مع العملاء على منصات التواصل.');
            }}
            className="bg-indigo-600 hover:bg-indigo-700 text-white px-2.5 py-1 rounded-lg font-bold transition-colors flex items-center gap-1"
            title="نسخ الرابط"
          >
            نسخ الرابط
          </button>
