...
import { Building2, Save, MapPin, Receipt, Phone, Calculator, Plus, X, Database, Download, ShieldCheck, MessageSquare } from 'lucide-react';
              ...
              <div className="space-y-3">
                {clinics.map(c => (
                  <div 
                    ...
                    className={`p-3 border rounded-xl cursor-pointer transition-colors ${selectedClinic?.id === c.id ? 'border-indigo-600 bg-indigo-50/50' : 'border-slate-100 hover:border-indigo-300'}`}
                  >
            ...
            {!selectedClinic ? (
              ...
              <div className="bg-white rounded-2xl p-6 shadow-sm border border-slate-200 h-full">
                ...
                <button 
                  ...
                  className={`w-full font-bold rounded-xl py-3 shadow-sm transition-all flex items-center justify-center gap-2 mt-4 ${isBranchManagementAllowed ? 'bg-indigo-600 text-white hover:bg-indigo-700' : 'bg-slate-100 text-slate-400 cursor-not-allowed'}`}
                >
                  <Save size={18} />
                  {isBranchManagementAllowed ? 'حفظ وتحديث الإعدادات' : 'مغلق بطلب من مطور النظام'}
                </button>
