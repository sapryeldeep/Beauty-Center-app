...
export function WhatsappSettingsModal({ isOpen, onClose, targetClinicId }: Props) {
  ...
  const activeClinic = data.clinics.find(c => c.id === targetClinicId) || data.clinics[0];
  ...
  useEffect(() => {
    if (activeClinic) {
      setWhatsappNumber(activeClinic.whatsappNumber || '');
      setWhatsappTemplate(activeClinic.whatsappTemplate || defaultTemplate);
    }
  }, [targetClinicId, activeClinic]);

  if (!isOpen || !activeClinic) return null;

  const handleSave = () => {
    const updatedClinics = data.clinics.map(c => {
      if (c.id === activeClinic.id) {
        return {
          ...c,
          whatsappNumber,
          whatsappTemplate
    ...
    alert('تم حفظ إعدادات تذكير الواتساب وقوالب الرسائل بنجاح!');
    onClose();
  };
